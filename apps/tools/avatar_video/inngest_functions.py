...
@inngest_client.create_function(
    fn_id="avatar-video-worker",
    trigger=inngest.TriggerEvent(event="tools/avatar_video.generate"),
    concurrency=[inngest.Concurrency(limit=5, key="event.data.user_id")],
)
async def process_avatar_video_job(ctx: inngest.Context, step: inngest.Step) -> dict:
    data = ctx.event.data
    ...
    try:
        def render_avatar():
            ...
            object_name = f"videos/avatar/{user_id}/{task_id}.mp4"
            storage_service.upload_file_bytes(dummy_mp4, object_name, "video/mp4")
            return storage_service.get_presigned_url(object_name)

        video_url = await step.run("render-avatar-pipeline", render_avatar)

        @sync_to_async
        def mark_complete():
            record = AvatarVideoGeneration.objects.get(id=task_id)
        ...
        logger.error(f"Avatar video failed for task {task_id}: {e}")
        ...
        def mark_failed():
            try:
                ...
                pass
            WalletService.refund(user_id, standard_credits, premium_credits, "avatar_video", reason=str(e))
