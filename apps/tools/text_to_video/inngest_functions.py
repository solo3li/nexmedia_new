...
@inngest_client.create_function(
    fn_id="text-to-video-worker",
    trigger=inngest.TriggerEvent(event="tools/text_to_video.generate"),
    concurrency=[inngest.Concurrency(limit=5, key="event.data.user_id")],
)
async def process_text_to_video_job(ctx: inngest.Context, step: inngest.Step) -> dict:
    data = ctx.event.data
    ...
    try:
        def render_video():
            # In production, polls CrunAI (Veo 3.1 / Grok Imagine)
            dummy_mp4 = b"\x00\x00\x00\x20ftypisom\x00\x00\x02\x00isomiso2avc1mp41"
            object_name = f"videos/t2v/{user_id}/{task_id}.mp4"
            storage_service.upload_file_bytes(dummy_mp4, object_name, "video/mp4")
            return storage_service.get_presigned_url(object_name)

        video_url = await step.run("render-video-pipeline", render_video)

        @sync_to_async
        def mark_complete():
            record = TextToVideoGeneration.objects.get(id=task_id)
        ...
        logger.error(f"Text-to-Video generation failed for task {task_id}: {e}")
        ...
        def mark_failed():
            try:
                ...
                pass
            WalletService.refund(user_id, standard_credits, premium_credits, "text_to_video", reason=str(e))
